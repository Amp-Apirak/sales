            ...
            <div class="card">
                <div class="card-header p-2">
                    <ul class="nav nav-pills">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" id="task-tab" data-toggle="pill" href="#tasks" role="tab" aria-controls="tasks" aria-selected="false">
                                บริหารโครงการ
                            </a>
                        </li>
...
<div class="modal fade" id="taskModal" tabindex="-1" role="dialog" aria-labelledby="taskModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="taskModalLabel">เพิ่มงานใหม่</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="taskForm">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="taskStatus">สถานะ</label>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="taskProgress">ความคืบหน้า (%)</label>
                    ...
                    <div class="form-group">
                        <label for="taskUsers">ผู้รับผิดชอบ</label>
                        <select class="form-control select2" id="taskUsers" name="users[]" multiple>
...
<script>
    ...
    function openAddTaskModal() {
        ...
        // เปลี่ยนชื่อ Modal
        $('#taskModalLabel').text('เพิ่มงานใหม่');
    ...
    $(document).ready(function() {
        // เริ่มต้น Select2 พร้อมการตั้งค่า
        $('#taskUsers').select2({
            placeholder: 'เลือกผู้รับผิดชอบ',
            allowClear: true,
            width: '100%', // กำหนดความกว้าง
            dropdownParent: $('#taskModal') // กำหนดให้ dropdown แสดงใน Modal
        });

        // แก้ไขฟังก์ชัน loadUsers
        function loadUsers() {
            $.ajax({
                url: '../project/management/get_users.php',
                type: 'GET',
                success: function(response) {
                    ...
                    $('#taskUsers').html(options).trigger('change');
                },
        ...
        // เรียกใช้งาน loadUsers เมื่อเปิด Modal
    ...
    function saveTask() {
        ...
        // ส่งข้อมูลไปยัง save_task.php
        $.ajax({
            url: '../project/management/save_task.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
...
<script>
    // เพิ่มฟังก์ชันสำหรับโหลดและแสดงข้อมูล Tasks
    function loadTasks() {
        $.ajax({
            url: '../project/management/get_tasks.php',
            type: 'GET',
            data: {
                project_id: '<?php echo $project_id; ?>'
            },
    ...
    function renderTaskTable(tasks) {
        ...
        tasks.forEach(task => {
            renderTaskRow(task, tbody, 0);
        });
    }

    function renderTaskRow(task, tbody, level) {
            ...
            <td>
                <div class="btn-group">
                    ...
                    <button type="button" class="btn btn-success btn-sm" 
                            onclick="addSubTask('${task.task_id}')">
    ...
    `;
        ...
        if (task.subtasks && task.subtasks.length > 0) {
            task.subtasks.forEach(subtask => {
                renderTaskRow(subtask, tbody, level + 1);
            });
        }
    }
    ...
    function editTask(taskId) {
        $.ajax({
            url: '../project/management/get_task_details.php',
            type: 'GET',
            data: {
                task_id: taskId
            },
    ...
    function fillTaskForm(task) {
        ...
        if (task.assigned_users) {
            $('#taskUsers').val(task.assigned_users.split(','));
            $('#taskUsers').trigger('change');
        }
    }
    ...
    function addSubTask(parentTaskId) {
        ...
        $('#taskModalLabel').text('เพิ่ม Sub Task');
        $('#taskModal').modal('show');
    }
    ...
    function deleteTask(taskId) {
        if (confirm('คุณแน่ใจหรือไม่ที่จะลบงานนี้?')) {
            $.ajax({
                url: '../project/management/delete_task.php',
                type: 'POST',
                data: {
                    task_id: taskId,
    ...
    $('a[href="#tasks"]').on('shown.bs.tab', function(e) {
        loadTasks();
    });
</script>